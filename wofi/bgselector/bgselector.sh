...
while read -r img; do
    ...
    # (
        if [[ "$img" =~ \.(gif|GIF)$ ]]; then
            convert "$img[0]" -strip -thumbnail 330x540^ -gravity center -extent 330x540 -quality 80 +repage "$cache_file" 2>/dev/null
        else
            convert "$img" -strip -thumbnail 330x540^ -gravity center -extent 330x540 -quality 80 +repage "$cache_file"
        fi
...
done < "$CACHE_INDEX" > "$wofi_input"

# Show wofi and get selection
selected=$(wofi -dmenu -show-icons -config "$HOME/.config/wofi/bgselector/style.rasi" < "$wofi_input")
cat "$wofi_input"
# rm "$wofi_input"
