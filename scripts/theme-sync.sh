...
detect_theme_from_wallpaper() {
  ...
  log_info "wallpaper_path: $wallpaper_path"
...
main() {
  ...
  log_info "unlink >/.local/wallpaper"
  unlink ~/.local/wallpaper
  log_info "ln -s $wallpaper_path ~/.local/wallpaper"
  ln -s "$wallpaper_path" ~/.local/wallpaper
  ...
  if [[ $theme_changed -eq 1 ]]; then
    ...
    send_notification "Theme Manager" "Theme $detected_theme" "" "normal" "preferences-desktop-theme"
  else
