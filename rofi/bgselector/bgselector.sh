...
while read -r img; do
    ...
    (
        if [[ "$img" =~ \.(gif|GIF)$ ]]; then
            ...
            convert "$img" -strip -thumbnail 330x540^ -gravity center -extent 330x540 -quality 80 +repage "$cache_file" 2>/dev/null
        fi
...
if [ -n "$selected" ]; then
    ...
    if [ -f "$selected_path" ]; then
        swww img "$selected_path" -t random --transition-duration 1 --transition-fps 60 &
        sleep 0.2
